 ...
 */
class tx_fsmiprotocols_view_list extends tx_fsmiprotocols_view_base {
	protected $Display;
	protected $accessObj;
	public $year          = 0;
	...
	function printProtocols ($protocolUIDs, $accessObj) {
		...
		foreach ($protocolUIDs as $protocol)	{
			$protocolDATA = t3lib_BEfunc::getRecord('tx_fsmiprotocols_list', $protocol);

			/* print list of all protocols for this year */
			// if there is really a plaintext protocol
			$contentProtocolList .= '
				<li style="list-style-image: url('.tx_fsmiprotocols_div::imgPath.'/document.png)" '.$this->pi_classParam('listrowField-meeting-date').'>';
			...
			if ($this->accessObj->isAccessAllowedProtocols($protocolDATA['meeting_date']))
				if ($protocolDATA['type']==tx_fsmiprotocols_div::kPROTOCOL_TYPE_PDF)
				...
				if ($this->accessObj->isAccessAllowedDocuments($protocolDATA['meeting_date']))
					foreach($documents as $documentDATA) {
						if ($this->accessObj->isAccessAllowedDocuments($protocolDATA['meeting_date'],$documentDATA['uid']))
							$contentProtocolList .= '<li style="margin-left:20px; list-style-image: url('.tx_fsmiprotocols_div::imgPath.'/file_additional.png)">'.
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fsmi_protocols/views/class.tx_fsmiprotocols_view_list.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fsmi_protocols/views/class.tx_fsmiprotocols_view_list.php']);
}
