 ...
 */
class tx_fsmiprotocols_view_single extends tx_fsmiprotocols_view_base {
	protected $Display;
	public $year          = 0;
	 ...
	 * @param	$accessObj object of type tx_fsmiprotocols_access that is preconfigured with access information
	...
	function printProtocol($protocolUID, $accessObj)	{
		...
		if ($protocolDATA['deleted']==1 || $protocolDATA['hidden']==1)
		...
		$content .= ($protocolDATA['meeting_time']!=0? '<li>'.$this->pi_getLL('time').': '.	// this time are seconds at day
			...
			tx_fsmiprotocols_div::twoDigits(intval( ($protocolDATA['meeting_time']%3600) / 60)).'</li>': '');
		...
		if ($this->Display['ShowAgendaElement']) {
			...
			if ($protocolDATA['agenda']!='' && $protocolDATA['agenda_preliminary']==1) {
				if ($accessObj->isAccessAllowedAgendaPreliminary($protocolDATA['meeting_date'])==true) {
					$content .= '<h3><a name="fsmiprotocols_agenda">'.$this->pi_getLL('preliminary_agenda').'</a></h3>';
					$content .= '<div>'.$this->showTextareaContentRTE($protocolDATA['agenda']).'</div>';
				}
				else
					$content .= tx_fsmiprotocols_notifier::printNotification(
			...
			elseif ($protocolDATA['agenda']!='' && $protocolDATA['agenda_preliminary']==0) {
				if ($accessObj->isAccessAllowedAgenda($protocolDATA['meeting_date'])==true) {
					$content .= '<h3><a name="fsmiprotocols_agenda">'.$this->pi_getLL('agenda').'</a></h3>';
					$content .= '<div>'.$this->showTextareaContentRTE($protocolDATA['agenda']).'</div>';
				}
				else
					$content .= tx_fsmiprotocols_notifier::printNotification(
		...
		if ($protocolDATA['protocol']!='' || $protocolDATA['protocol_pdf']!='') {
			if ($protocolDATA['not_admitted']==1)
				$content .= '<h4><a name="fsmiprotocols_protocol">'.$this->pi_getLL('not_admitted_protocol').'</a></h4>';
			else
				$content .= '<h4><a name="fsmiprotocols_protocol">'.$this->pi_getLL('protocol').'</a></h4>';
			if ($accessObj->isAccessAllowedProtocols($protocolDATA['meeting_date'])==false) {
				...
				// print text-protocol
				if ($protocolDATA['type']==tx_fsmiprotocols_view_base::kPROTOCOL_TYPE_PLAIN &&
				...
				// give PDF protocol
				elseif ($protocolDATA['type']==tx_fsmiprotocols_view_base::kPROTOCOL_TYPE_PDF && $protocolDATA['protocol_pdf']!='')
					$content .= '<p>'.$this->pi_getLL('protocol_as_pdf_descr').': <a href="'.tx_fsmiprotocols_div::uploadFolder.$protocolDATA['protocol_pdf'].'">
						'.$this->pi_getLL('meeting-protocol').'</a></p>';
			}
		}
		else // no protocol exists
			$content .= tx_fsmiprotocols_notifier::printNotification($this->pi_getLL('no_protocol'),$this->pi_getLL('no_protocol_long'));
			...
			if (count($documents)>0) {
				if ($accessObj->isAccessAllowedDocuments($protocolDATA['meeting_date'])==false) {
					...
					foreach($documents as $documentDATA) {
							$content .= '<li style="margin-left:20px; list-style-image: url('.tx_fsmiprotocols_div::imgPath.'/file_additional.png)">'.
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fsmi_protocols/views/class.tx_fsmiprotocols_view_single.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fsmi_protocols/views/class.tx_fsmiprotocols_view_single.php']);
}
