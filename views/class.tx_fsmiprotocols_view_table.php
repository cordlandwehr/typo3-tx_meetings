 ...
 */
class tx_fsmiprotocols_view_table extends tx_fsmiprotocols_view_base {
	...
	function printProtocolTableLine ($protocolUID, $oddLine=true) {
		...
		$this->year = tx_fsmiprotocols_div::dateToTenureYear($protocolDATA['meeting_date'],$protocolDATA['sticky_date']);

		// here the real code starts
		$contentProtocolTable = '<tr '.($oddLine? 'class="fsmiprotocols_row_odd"':'class="fsimprotocols_row_even"').'>';
		// date, room, time
		...
		if ($this->Display['ShowAgendaElement']) {
			...
			if ($protocolDATA['agenda']!='')
				$contentProtocolTable .= $this->pi_linkToPage(
															...
															$GLOBALS['TSFE']->id.'#fsmiprotocols_agenda',
															'',
															array(
																$this->extKey.'[showUid]' => $protocolDATA['uid'],
																$this->extKey.'[year]' => $this->year,
			...
			if ($protocolDATA['protocol']!='' || $protocolDATA['protocol_pdf'])
				$contentProtocolTable .= $this->pi_linkToPage($this->pi_getLL('meeting-protocol'), $GLOBALS['TSFE']->id.'#fsmiprotocols_protocol','',
															array(
																$this->extKey.'[showUid]' => $protocolDATA['uid'],
																$this->extKey.'[year]' => $this->year,
															)).
			...
			if ($this->accessObj->isAccessAllowedDocuments($protocolDATA['meeting_date']))
				foreach($documents as $documentDATA) {
					if ($this->accessObj->isAccessAllowedDocuments($protocolDATA['meeting_date'],$documentDATA['uid']))
						...
						$contentProtocolTable .= '<img title="'.$documentDATA['name'].'" src="'.tx_fsmiprotocols_div::imgPath.'/file_additional.png" alt="'.
	...
	function printProtocols ($protocolUIDs, $accessObj) {
		...
		$content = '';
		$content .= '<table class="fsmiprotocols_table">';
		$content .= '<tr>';
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fsmi_protocols/views/class.tx_fsmiprotocols_view_table.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fsmi_protocols/views/class.tx_fsmiprotocols_view_table.php']);
}
