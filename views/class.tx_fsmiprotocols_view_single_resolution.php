 ...
 */
class tx_fsmiprotocols_view_single_resolution extends tx_fsmiprotocols_view_base {
	...
	function printSingleResolution ($resolutionUID, $accessObj) {
		...
		if ($accessObj->isAccessAllowedResolutions($protocolDATA['meeting_date'])==false)
			$content .= tx_fsmiprotocols_notifier::printNotification('Beschluss Zugriff','Sie haben nicht die Rechte um auf diesen Beschluss zuzugreifen.');
		else
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fsmi_protocols/views/class.tx_fsmiprotocols_view_single_resolution.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fsmi_protocols/views/class.tx_fsmiprotocols_view_single_resolution.php']);
}
