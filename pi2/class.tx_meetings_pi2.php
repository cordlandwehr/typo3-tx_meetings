 ...
 */
class tx_meetings_pi2 extends tslib_pibase {
	var $prefixId      = 'tx_meetings_pi2';		// Same as class name
	var $scriptRelPath = 'pi1/class.tx_meetings_pi2.php';	// Path to this script relative to the extension dir.
	...
	function listAll ($committee) {
		...
		if (t3lib_div::_POST('protocol_changes') && $protocolPost['protocol_uid']) {
			$protocolDB = t3lib_BEfunc::getRecord('tx_meetings_list', intval($protocolPost['protocol_uid']));
		...
		while($res && $row = mysql_fetch_assoc($res)) {
			...
			$content .= '<td>';
			if ($row['hidden']==1)
				$content .= tx_meetings_div::printImageProtocolState(tx_meetings_div::kIMAGE_HIDDEN);
			else {
				if ($committeeDATA['disclosure'] == tx_meetings_div::kDISCLOSURE_REVIEWERS
					  && !($row['reviewer_a']  && $row['reviewer_b']) )
					$content .= tx_meetings_div::printImageProtocolState(tx_meetings_div::kIMAGE_REVIEW_MISSING);
				else
					$content .= tx_meetings_div::printImageProtocolState(tx_meetings_div::kIMAGE_PUBLISHED);
			}
			$content .= '</td>';
	...
	function DBNewProtocol () {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_INSERTquery(	'tx_meetings_list',
	...
	function DBUpdateProtocolData ($protocol) {
		...
		$GLOBALS['TYPO3_DB']->exec_UPDATEquery(	'tx_meetings_list',
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/meetings/pi2/class.tx_meetings_pi2.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/meetings/pi2/class.tx_meetings_pi2.php']);
}
